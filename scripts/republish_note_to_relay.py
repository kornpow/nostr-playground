...
from nostr_sdk import Client, Event, EventId, Filter
...
async def publish_event_to_relays(event: Event, relay_urls: list, timeout: int = 10):
            ...
            try:
                ...
                if hasattr(response, "success") and hasattr(response, "failed"):
                    ...
                    if relay_url in getattr(response, "success", []):
                        ...
                        relay_error = response.failed[relay_url]
                if relay_success:
    ...
    return {"success": success_relays, "failed": failed_relays, "errors": errors}
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Republish a Nostr note to relays where it's missing"
    )
    ...
    parser.add_argument(
        "--target-relays",
        nargs="+",
        help="Specific relays to publish to (overrides JSON)",
    )
    parser.add_argument(
        "--all-relays",
        action="store_true",
        help="Publish to all relays in relays.txt file",
    )
    parser.add_argument(
        "--relay-file",
        default="relays.txt",
        help="File containing relay list (default: relays.txt)",
    )
